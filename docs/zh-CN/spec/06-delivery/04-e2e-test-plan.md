...
- **步骤**：1) 打开“设置 → 智能体 → 子智能体”，点击“新建子智能体”，确认表单上方有
  ...
  `vendorKey-or-name/modelId`。5) 确认选择器里没有“自定义（provider/model）…”入口，
  4a) 配置一个显示名含空格的自定义端点（例如「My Gateway」），确认选择器会列出它；
  选中后确认表单能保存，固定值为 `<显示名>/<modelId>`，且保存按钮保持可用——
  选择器与草稿校验绝不能对「什么能保存」产生分歧。
  也没有任何可手填模型 ID 的输入框；再选择“沿用当前会话的模型”，确认模型值为空。
